...
internal class KonanForwardDeclarationModuleDeserializerK2(
    ...
    private val declaredPackages = mutableMapOf<FqName, IrExternalPackageFragment>()
    ...
    override fun tryDeserializeIrSymbol(idSig: IdSignature, symbolKind: BinarySymbolData.SymbolKind): IrSymbol? {
        if (idSig !is IdSignature.CommonSignature) return null
        ...
        return declaredClasses.computeIfAbsent(idSig) {
    ...
    private fun buildForwardDeclarationClassStub(signature: IdSignature.CommonSignature): IrClass? {
        ...
        val superClassSignature = IdSignature.CommonSignature(
            kind.superClassId.packageFqName.asString(), kind.superClassId.relativeClassName.asString(),
            null, 0, null
        )
    ...
    private fun getOrCreateContainingPackage(packageFqName: FqName): IrExternalPackageFragment {
        return declaredPackages.computeIfAbsent(packageFqName) {
