    ...
    override fun check(declaration: FirRegularClass) {
        ...
        if (primaryConstructor?.source?.kind is KtRealSourceElementKind) {
            if (isJvmInlineMultiFieldEnabled || isExtendedValueClass) {
                ...
                val jvmInlineAnnotation = context.session.jvmInlineAnnotationClassId
                ...
                if (primaryConstructorParametersByName.size > 1 && !hasJvmInlineAnnotation && valueModifier != null) {
                    reporter.reportOn(
                        valueModifier.source, FirErrors.UNSUPPORTED_FEATURE, LanguageFeature.ValueClasses to context.languageVersionSettings
                    )
                } else {
                    reporter.reportOn(
