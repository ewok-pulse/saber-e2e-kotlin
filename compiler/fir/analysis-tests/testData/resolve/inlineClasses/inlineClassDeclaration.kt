...
class A {
    ...
    inner <!UNSUPPORTED_FEATURE("The feature \"value classes\" is experimental and should be enabled explicitly. This can be done by supplying the compiler argument '-XXLanguage:+ValueClasses', but note that no stability guarantees are provided."), VALUE_CLASS_NOT_TOP_LEVEL!>value<!> class D(val x: Int)
}
...
<!UNSUPPORTED_FEATURE("The feature \"value classes\" is experimental and should be enabled explicitly. This can be done by supplying the compiler argument '-XXLanguage:+ValueClasses', but note that no stability guarantees are provided."), VALUE_CLASS_CANNOT_BE_CLONEABLE!>value<!> class CloneableClass1(val x: Int): Cloneable
