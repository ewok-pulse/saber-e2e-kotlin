 ...
 *
 *  - It has visibility `internal`, `private`, or `protected`.
...
abstract class AbstractAnalysisApiInternalApiTest : AbstractAnalysisApiCodebaseValidationTest() {
    ...
    private fun collectViolations(declaration: KtDeclaration, result: MutableList<CodebaseDeclarationAnnotation>) {
        ...
        // visibility (internal/private/protected) already removes it from the public API surface.
        if (declaration is KtClassOrObject && declaration.isPublic) {
    ...
    private fun isViolation(declaration: KtDeclaration): Boolean =
        when {
            ...
            !declaration.isPublic -> false
            declaration.hasModifier(KtTokens.OVERRIDE_KEYWORD) -> false
    ...
    private fun buildErrorMessage(violations: List<CodebaseDeclarationAnnotation>): String {
        ...
        return """
            ...
            |marked with `internal`, `private`, or one of `@KaImplementationDetail`, `@KaExperimentalApi`, `@KaPlatformInterface`,
            |`@KaNonPublicApi`, `@KaIdeApi`, or `@LLFirInternals`.
            |
