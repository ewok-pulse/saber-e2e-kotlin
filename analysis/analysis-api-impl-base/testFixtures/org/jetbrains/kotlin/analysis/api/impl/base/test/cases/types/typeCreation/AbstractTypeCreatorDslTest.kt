    ...
    override fun doTestByMainFile(mainFile: KtFile, mainModule: KtTestModule, testServices: TestServices) {
        val actualText = analyzeForTest(mainFile) {
            ...
            TypeCreatorDslTestRenderer.render(
                ...
                testDataFileName = testInfo.methodName.removePrefix("test").decapitalizeAsciiOnly()
            )
        }
...
object TypeCreatorDslTestRenderer {
    ...
    fun render(
        ...
        val value = produceValue(directoryName, testDataFileName, caretToType).let(processor)
        return render(value)
    }
    ...
    private fun render(value: Any?): String {
        ...
        return buildString {
            when (value) {
                is KaType -> {
                    appendLine("KaType")
                    appendLine(KaDebugRenderer(renderTypeByProperties = true).renderType(session, value))
                    appendLine()
                ...
                is KaTypeArgumentWithVariance -> {
                    ...
                    appendLine("Type: ${KaDebugRenderer(renderTypeByProperties = true).renderType(session, value.type)}")
                    appendLine()
    ...
    context(session: KaSession)
    private fun produceValue(directoryName: String, testDataFileName: String, caretToType: Map<String, KaType>): Any? {
        val testClass = testClassesMapping.getOrElse(directoryName) {
            error("Unable to find test class for $directoryName/$testDataFileName")
        }

        val testClassInstance = testClass.primaryConstructor?.call(session, caretToType)
            ?: error("Cannot construct test class for $directoryName")

        val testMethodName = "test" + testDataFileName.capitalizeAsciiOnly()

        return testClass.members.filterIsInstance<KFunction<*>>()
            .single { it.name == testMethodName }
    ...
    private val testClassesMapping: Map<String, KClass<out TestCases>> = mapOf(
        ...
        "varargArrayType" to VarargArrayTypeCreatorDslTestCases::class,
        "typeParameterType" to TypeParameterTypeCreatorDslTestCases::class,
        "typeProjection" to TypeProjectionCreatorDslTestCases::class,
