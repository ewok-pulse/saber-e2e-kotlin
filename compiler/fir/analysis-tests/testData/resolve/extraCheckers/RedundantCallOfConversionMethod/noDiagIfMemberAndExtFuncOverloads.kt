...
fun test() {
    val c = C()
    c.test(1.<!REDUNDANT_CALL_OF_CONVERSION_METHOD!>toInt()<!>) // The `toInt` call forces resolving to the extension, the REDUNDANT_CALL_OF_CONVERSION_METHOD shouldn't be reported here
    c.test((2 + 3).<!REDUNDANT_CALL_OF_CONVERSION_METHOD!>toInt()<!>) // Resolving to the extension, no REDUNDANT_CALL_OF_CONVERSION_METHOD
