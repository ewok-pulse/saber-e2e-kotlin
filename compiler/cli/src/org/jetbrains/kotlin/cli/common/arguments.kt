...
fun CompilerConfiguration.setupCommonArguments(
    ...
    if (arguments.additionalIrCheckers.isNotEmpty() && irVerificationMode == IrVerificationMode.NONE) {
        this.report(
            COMPILER_ARGUMENTS_WARNING,
            "'-Xadditional-ir-checkers' has no effect unless '-Xverify-ir=warning' or '-Xverify-ir=error' is specified"
        )
    }
    put(CommonConfigurationKeys.ADDITIONAL_IR_CHECKERS, arguments.additionalIrCheckers.toList())
