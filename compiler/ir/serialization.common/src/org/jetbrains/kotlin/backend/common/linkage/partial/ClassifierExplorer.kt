...
internal class ClassifierExplorer(
    ...
    private fun IrValueParameter.exploreAnnotationConstructorParameter(
        ...
        when {
            ...
            parameterClass.isEnumClass || parameterType.isPrimitiveType() || parameterType.isArray() ||
                    parameterType.isUnsignedType() || parameterType.isUnsignedArray() -> return null // Permitted.
    ...
    private fun IrClass.exploreSuperClasses(superTypeSymbols: Set<IrClassSymbol>): Unusable? {
        if (isInterface) {
            ...
            if (!superClass.isAny()
                && !superClass.isEnumClass // Enum class can't be explicitly inherited, only valid enum class can inherit it.
            ) {
